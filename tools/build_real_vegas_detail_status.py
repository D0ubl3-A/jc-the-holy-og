...
def main():
 ...
 assert 'movePlayerSolid(' in mission_runtime
 assert 'buildTileColliders(' in mission_runtime
 assert 'loadRoadRuntime(' in mission_runtime
 assert 'loadOneTile(' in mission_runtime
 assert 'conformRoadTileToGlb(key)' in mission_runtime
 assert mission_runtime.count('conformRoadTileToGlb(key);') >= 2
 ...
 assert 'JC_CHARACTER_ATLAS_DATA_URL' in mission_runtime
 assert Path('jc-the-holy-og-assets/jc-character-atlas-v2-data.js').is_file()
 for part in range(1,5): assert Path(f'jc-the-holy-og-assets/jc-avatar-data/part{part:02d}.js').is_file()
 assert int(mission.get('buildings',0))>0
 assert int(mission.get('roads',0))>0
 status={
  ...
  'deadline_vertical_slice':{'status':'PASS','buildings_loaded_and_finalized':mission['buildings'],'roads':mission['roads'],'jc_character':'modern realistic four-direction black tactical JC atlas with cross pendant in 3D world','flight_controls':'F toggle; mouse/pointer-lock crosshair aim; camera-relative WASD; Space ascend; C/Ctrl descend; Shift boost','solar_flight':'hold left mouse, R, or mobile CHARGE to absorb sunlight into a persistent 0-100 meter','destination_hyperspeed':'Q/mobile TARGET cycles named source landmarks; right mouse/H/mobile HYPER consumes 40 solar and launches to the selected rooftop','building_finish':'deterministic procedural facade, windows and roof treatment on every loaded building'},
  'remaining_major_realism_work':['exact facade/architecture evidence per building','property/business identity and address resolution','real entrances and interiors','lane-level/right-of-way/sidewalk/signals citywide','continue runtime QA for road/vehicle/player surface conformity across all streamed GLB tiles','expand runtime selection beyond 43,500 while preserving mobile frame budget'],
